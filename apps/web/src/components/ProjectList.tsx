...
function NewProjectDialog({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <form className="modal" onSubmit={(event) => void submit(event)}>
      ...
      <label>Mode<select value={mode} onChange={(event) => setMode(event.target.value as ProjectMode)}><option value="generative">Generative</option><option value="raster">Raster image</option><option value="svg">SVG import</option><option value="map">OpenStreetMap</option></select></label>
