...
export function FluidNCControlDialog({ onClose, capture }: { onClose: () => void; capture?: { label: string; onCapture: (position: FluidNCPosition) => void } }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="modal machine-control" role="dialog" aria-modal="true" aria-labelledby="machine-control-title">
      ...
      {capture && <div className="capture-panel"><div><strong>Capture {capture.label}</strong><span>Jog the brush over the centre of the paint, then save the current work X/Y.</span></div><button className="button primary" type="button" disabled={connection !== 'connected' || !status.workPosition} onClick={() => status.workPosition && capture.onCapture(status.workPosition)}><Target size={16} /> Use current X/Y</button>{connection === 'connected' && !status.workPosition && <small>Waiting for FluidNC work coordinates…</small>}</div>}
